...
class GetDeviceName(APIView):
    def get(self, request, *args, **kwargs):
        ...
        from alba_security_app.devices import get_device_name_by_ip
        name = get_device_name_by_ip(ip)
        return JsonResponse({"name": name})

class GetDevices(APIView):
    def get(self, request, format=None):
        try:
            result = subprocess.run(['python3', '/app/alba_security_app/devices.py'], capture_output=True, text=True)
...
class ScanDevicesStart(APIView):
    def get(self, request, format=None):
        try:
            result = subprocess.run(['python3', '/app/alba_security_app/scan_devices.py'], capture_output=True, text=True, timeout=50)
            output = result.stdout
            if result.stderr:
                output += "\nError Output:\n" + result.stderr
            return JsonResponse({'status': 'success', 'output': output})
            ...
            output = "No se han detectado conexiones externas."
            return JsonResponse({'status': 'success', 'output': output})
...
class CreateConnection(APIView):
    def post(self, request, *args, **kwargs):
        ...
        first_device_object=Device.objects.filter(model=first_device).first()
        second_device_object=Device.objects.filter(model=second_device).first()
